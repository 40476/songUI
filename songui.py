...
- Shows a big error if playerctl is not installed.
- Limits CPU usage when idle.
- Buttons stay highlighted for (autorefresh_interval*0.60) seconds after being pressed.
- Animated bar moves slower.
- If --device is specified, attempts to connect at startup.
- "Reconnect" is a keyboard key ([r]), available only when device not found.
- Every 5 seconds, if in bluetooth mode, alternates bluetoothctl player.scan between <value> and its opposite (alltracks/off).
- All command-line arguments have short forms.
- player.scan is NOT displayed in the UI.
